...
latest version:
    ...
    cd build
	cmake .. ninja ctest -j 8 ninja install
...
SWI-Prolog executables will try to [find the correct "home"
directory](https://www.swi-prolog.org/pldoc/man?section=findhome). The
script above will work assuming step 4 will be used when finding SWIPL's
home location. However, if `SWI_HOME_DIR` or `SWIPL` are defined, a
mismatch could arise between the executable and the SWIPL "home"
directory being used, causing errors like the following one:

```shell
$ swipl # using $HOME/bin/swipl as created by swipl-activate
[FATAL ERROR: at Sat May 25 18:09:51 2024
	/usr/lib/swi-prolog/boot.prc: incompatible VM-signature (file: 0x408bfff4; Prolog: 0x6ed28fea)]
[1]    3773339 IOT instruction (core dumped)  ./src/swipl
```

Either change or delete the environment variables or explicitly state the
SWIPL's home folder, e.g.:

```shell
$ swipl --home=/path/to/swipl-devel/build/home                                                                        ─╯
```
