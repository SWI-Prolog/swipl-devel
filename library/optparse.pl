...
/*
    Revision history:
    0.20 (2011-04-27) * public release 
                      * all fields now are sized dynamically
                      * the meta-type-default fields are now merged and 
                          formatted just like library record, that is 
                          x:integer=3, f:float=0.1;
                          if meta is not given, only integer=3, float=0.1 
                          is seen.
                      * the user can now specify line breaks and indent in 
                          the help text
                      * opt_help/2 now is opt_help(OptsSpec, Help, []);
                          i.e, opt_help/3 exposes some formatting options  
                          (currently not exported, but could be)
                      * internal changes: parsing rewritten entirely with DCG
    
    0.15 (2011-04-21) * long help lines now word wrap
                      * added special default value '_' to create uninstantiated
                          variable
                      * wrote some tests for opt_parse
                      * internal changes: all defaults added from beginning, 
                          makes subsequent checking and access much easier 
                      * member -> memberchk for deterministic uses (many)
                      * 0'<char> syntax now used for char codes
                      * boolean implicits now added with typecheck, much safer
                      
    0.14 (2011-04-20) * renamed (again): 
                          entire library argparse -> optparse
                          pred parse_args -> opt_parse
                          pred get_parsed_args -> opt_arguments
                          pred args_help -> opt_help
                      * slight cleanup in error handling:
                          assertion/1 for, well, assertions
                          must_be/2, is_list/1 for type checking
                      * wrote some tests for validate_opts_spec
                          
    0.13 (2011-04-18) * fixed documentation bug
    
    0.12 (2011-04-17) * added special default value to parse option 
                        output_functor('OPTION')
                      * added parse option allow_empty_flag_spec(Bool)
                      * added revision history
                      * updated documentation 
                      
    0.11 (2011-04-15) * changed license to LGPL for general swi compatibility
                      * renamed module to argparse.pl (not many swi specifics) 
                      * added compatibility notes for other prologs
                      * made naming consistent with the pedagogically documented 
                        python optparse (NonOpts -> PositionalArgs)
                      * updated documentation
                      * changed internal representation to codes from chars,
                        :- set_prolog_flag(double_quotes, chars) removed 
                      * all parsing errors now throw syntax_error
                      * -s=foo can only cause confusion and is now invalid
                      
    0.10 (2011-04-14) * public release (as swi_argparse.pl)
...
This module helps in building a command-line interface to an application.
In particular, it provides functions that take an option specification and a 
list of atoms, probably given to the program on the command line,
and return a parsed representation (a list of the customary Key(Val) by default; 
or optionally, a list of =|Func(Key, Val)|= terms in the 
style of current_prolog_flag/2). It can also synthesize a simple help text 
from the options specification.
...
Very briefly, _arguments_ is what you provide on the command line
and for many prologs show up as a list of atoms =|Args|= 
in =|current_prolog_flag(argv, Args)|=.
For a typical prolog incantation, they can be divided into 

    * _|runtime arguments|_, which controls the prolog runtime; conventionally,
     they are ended by '--'; 
    * _options_, which are key-value pairs (with a boolean value possibly implicit)
    intended to control your program in one way or another; and 
...
Positional arguments are in particular used for mandatory arguments without which
your program won't work and for which there are no sensible defaults
(e.g,, input file names). Options, by contrast, offer flexibility by
letting you change a default setting. Options are optional not only 
by etymology: this library has no notion of
...
This is just a list of individual option specifications.
One of those, in turn, is a list of ground prolog terms 
in the customary =|Name(Value)|= format.  
The following terms are recognized (any others raise error).
	...
	* shortflags(ListOfFlags) 
	...
	current option
	(=|-s , -K|=, etc). Uppercase letters must be quoted. 
	...
	* longflags(ListOfFlags) 
	...
	(=|--verbose, --no-debug|=, etc). They are basically a more readable
	alternative to short flags, except
	
	1. long flags can be specified as =|--flag value|= or =|--flag=value|= (but not
	   as =|--flagvalue|=); short flags as  =|-f val|= or =|-fval|= (but not
	   =|-f=val|=)
	2. boolean long flags can be specified as =|--bool-flag|= or =|--bool-flag=true|= or 
	   =|--bool-flag true|=; and they can be negated as =|--no-bool-flag|= or
	   =|--bool-flag=false|= or =|--bool-flag false|=.
	
    Except that shortflags must be single characters, the distinction between 
    long and short is in calling convention,
    not in namespaces. Thus, if you have =|shortflags([v])|=, you can use it 
	...
	* meta(Meta) 
	...
	and is the name (an atom) of the metasyntactic variable (possibly) appearing
	in it together with type and default value (e.g, =|x:integer=3|=, 
	=|interest:float=0.11|=).  
	It may be useful to have named variables (=|x|=, =|interest|=)
	in case you wish to mention them again in the help text. 
    If not given the =|Meta:|= part is suppressed -- see example below.
	...
	* help(Help) 
	Help is (usually) an atom of text describing the option in the help text. 
	This term 
	is optional (but obviously strongly recommended for all options which
	have flags).
	
	Long lines are subject to basic word wrapping -- split on white space,
    reindent, rejoin. However, you can get more control by supplying the
    line breaking yourself: rather than a single line of text, you can 
    provide a list of lines (as atoms). If you do, they will be 
    joined with the appropriate indent but
    otherwise left untouched (see the option =|mode|= in the example below). 
     	
	
Absence of mandatory option specs or the presence of more than one for
a particular option throws an error, as do unknown or incompatible types. 

As a concrete example from a fictive application, suppose we want the
following options to be read from the command line
(long flag(s), short flag(s), meta:type=default, help)
...
This interface is described by the following options specification (order
between the specifications of a particular option is irrelevant).
...
==
The help text above was accessed by =|opt_help(ExamplesOptsSpec, HelpText)|=.
The options appear in the same order as in the OptsSpec.

Given =|ExampleOptsSpec|=, a command line (somewhat syntactically inconsistent, 
in order to demonstrate different calling conventions)
may look as follows
==
ExampleArgs = [ '-d5'
...
(of the implicit type 'term'), but
no corresponding flags in =|OptsSpec|=. Thus it can't be set from the command line.
The rest of your program doesn't need to know
that, of course. This provides an alternative to the common practice
of asserting such hard-coded parameters under a single predicate
(for instance =|setting(path, '/some/file/path')|=), with the advantage that
you may seamlessly upgrade them 
to command-line options, should you one day find this a good idea.
Just add an appropriate flag or two and a line of help text. Similarly, 
suppressing an option in a cluttered interface amounts to commenting out the 
flags.


opt_parse/5 allows more control through an additional argument list. For instance, 
...
---+ Notes and tips
    ...
    * Unknown flags (not appearing in OptsSpec) will throw errors. 
    This is usually a Good Thing. Sometimes, however,
    you may wish to pass along flags to an external program (say, one called
    by shell/2), and it means duplicated effort and a maintenance headache 
    to have to specify all possible flags for the external program explicitly
    (if it even can be done). On the other hand, simply taking all unknown flags 
    as valid makes error checking much less efficient and
    identification of positional arguments uncertain.
    A better solution is to collect all arguments intended for passing along
    to an indirectly called program as a single argument, probably as an 
    atom (if you don't need to inspect them first) or as a prolog term (if you do). 

  
---+ Compatibility note

This library is written for swi-prolog (http://www.swi-prolog.org). 
Most predicates are either iso or otherwise widely known. Possible exceptions
include =|option/3, merge_options/3|= from the swi library(option);
=|term_to_atom/2|= ; and =|format(+Output, +Format, +Arguments)|= 
when Output as a non-stream. 
...
%
%    Convenience predicate, assuming that command-line arguments
%    can be accessed by 
%    =|current_prolog_flag|= (as in swi-prolog). For other access mechanisms
%    and/or more control,
%    get the args and pass them as a list of atoms to opt_parse/4 or 
%    opt_parse/5 instead.
%
...
%    has taken its argument (filling in =|true|= or =|false|= for 
%    booleans). There are no restrictions on non-dashed 
%    arguments and they may go anywhere (although it is good practice
%    to put them last). 
...
%    Any runtime arguments (typically terminated by '--') are assumed
...
%    booleans). There are no restrictions on non-dashed 
%    arguments and they may go anywhere (although it is good practice
%    to put them last). ParseOptions are 
%
%    * output_functor(Func)
...
%
%    Returns a help string (atom), synthesized from the help descriptions in 
%    OptsSpec.
