...
macro(configure_pgo pgo_tag)
  ...
  # Default link flags to CFLAGS
  if(NOT PGO_GENERATE_LINK_FLAGS)
    set(PGO_GENERATE_LINK_FLAGS ${PGO_GENERATE_FLAGS})
  endif()
  if(NOT PGO_USE_LINK_FLAGS)
    set(PGO_USE_LINK_FLAGS ${PGO_LINK_FLAGS})
  endif()
endmacro()

function(generate_pgo_data) # generate_pgo_data(targets...)
  string(REPLACE ";" " " gen_flags "${PGO_GENERATE_LINK_FLAGS}")
  foreach(t IN LISTS ARGV)
    set(tinstr "${t}${PGO_SUFFIX}")
    target_compile_options(${tinstr} PRIVATE ${PGO_GENERATE_FLAGS})
    if(${CMAKE_VERSION} VERSION_GREATER 3.12)
      target_link_options(${tinstr} PRIVATE ${PGO_GENERATE_LINK_FLAGS})
    else()
...
function(use_pgo_data) # use_pgo_data(targets...)
  ...
  string(REPLACE ";" " " use_flags "${PGO_USE_LINK_FLAGS}")
  foreach(t IN LISTS ARGV)
    target_compile_options(${t} PRIVATE ${PGO_USE_FLAGS})
    if(${CMAKE_VERSION} VERSION_GREATER 3.12)
      target_link_options(${t} PRIVATE ${PGO_USE_LINK_FLAGS})
    else()
