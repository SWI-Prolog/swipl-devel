...
#endif
#define Sword		AN
#define HASSYNTAX(c, s)	((char_flags[c] & s) ? 1 : 0)
#undef fail				/* conflict */
#endif
...
#ifdef __WATCOMC__
#define SIGN_EXTEND_CHAR(x) ((((int)(x)) << 24) >> 24)
#include <string.h>
#define bcopy(s,d,n)	memmove((d),(s),(n))
#define bcmp(s1,s2,n)	memcmp((s1),(s2),(n))
#define bzero(s,n)	memset((s),0,(n))
#include <malloc.h>		/* get alloca() */
#endif

#if defined(USG) || defined(sun) || defined(_POSIX_SOURCE)
#include <memory.h>
	...
	case '|':
	  ...
	  switch (c)
	    ...
	    case 's':	
	      ...
	      PATPUSH (syntaxspec);
	      PATFETCH (c);
	      PATPUSH (syntax_spec_code[c] & 0xff);
	      PATPUSH ((syntax_spec_code[c] >> 8 )& 0xff);
	      break;
...
re_compile_fastmap (struct re_pattern_buffer *bufp)
{
  ...
  while (p)
	...
	case syntaxspec:
	  k = *p++;
	  for (j = 0; j < (1 << BYTEWIDTH); j++)
	    if (HASSYNTAX (j, (enum syntaxcode) k))
	      fastmap[j] = 1;
	  break;
...
re_search_2 (struct re_pattern_buffer *pbufp, char *string1, int size1, char *string2, int size2, int startpos, register int range, struct re_registers *regs, int mstop)
{
  ...
  while (1)
	...
	return -1;
      ...
      val = re_match_2 (pbufp,
			(unsigned char *)string1, size1,
			(unsigned char *)string2, size2,
			startpos, regs, mstop);
      if (0 <= val)
	{
	  if (val == -2)
...
int
re_match (struct re_pattern_buffer *pbufp, char *string,
	  int size, int pos, struct re_registers *regs)
{
  return re_match_2 (pbufp, 0, 0,
  ...
  while (1)
	...
	case wordbound:
	  if (d == string1  /* Points to first char */
	    ...
	    break;
	  if ((HASSYNTAX (d[-1], Sword))
	      != (HASSYNTAX (d == end1 ? *string2 : *d, Sword)))
	    break;
	  goto fail;
	...
	case wordbeg:
	  if (d == end2  /* Points to end */
	      || (d == end1 && size2 == 0) /* Points to end */
	      || HASSYNTAX (* (d == end1 ? string2 : d), Sword)) /* Next char not a letter */
	    goto fail;
	  if (d == string1  /* Points to first char */
	      || !HASSYNTAX (d[-1], Sword))  /* prev char not letter */
	    break;
	  goto fail;
	...
	case wordchar:
	  PREFETCH;
	  if (!HASSYNTAX (*d++, Sword)) goto fail;
	  break;
