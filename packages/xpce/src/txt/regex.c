...
/*re->compiled->no_sub    = 0;*/
  re->compiled->translate = NULL;
...
{ if ( re->compiled != NULL )
  { if ( re->compiled->buffer != NULL )
    ...
    { unalloc(FASTMAPSIZE, re->compiled->fastmap);
      re->compiled->fastmap = NULL;
    }
  }
}
...
{ if ( re->compiled->buffer == NULL )
  { char *error;
    ...
    re->compiled->fastmap   = (optimize == ON ? alloc(FASTMAPSIZE) : NULL);
...
/*  re->registers->num_regs = 0;
    re->registers->start    = NULL;
    re->registers->end      = NULL; */
  }
...
{ TRY(compileRegex(re, DEFAULT));
  ...
  answer(ZERO/*toInt(re->compiled->re_nsub)*/);
}
