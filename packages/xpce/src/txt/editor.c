...
static Int		getMarginWidthEditor P((Editor));
static Int		getColumnEditor P((Editor, Int));
static Int		getLineNumberEditor P((Editor, Int));
static Int		getLengthEditor P((Editor));
static Int		normalise_index P((Editor, Int));
static FragmentCache	newFragmentCache P((Editor));
static void		freeFragmentCache P((FragmentCache));
static void		resetFragmentCache P((FragmentCache, TextBuffer));
static Any		Receiver P((Editor));
static status		caretEditor P((Editor, Int));
static status		IsearchEditor P((Editor, EventId));
static status		DabbrevExpandEditor P((Editor, EventId));
static status		centerWindowEditor P((Editor, Int));
static status		columnEditor P((Editor, Int));
static status		ChangedRegionEditor P((Editor, Int, Int));
static status		ChangedEditor P((Editor));
static status		appendKill P((CharArray));
static status		prependKill P((CharArray));
static status		geometryEditor P((Editor, Int, Int, Int, Int));
static status		ensureVisibleEditor P((Editor, Int, Int));
static status		ensureCaretInWindowEditor P((Editor));
static status		endIsearchEditor P((Editor));
static status		updateStyleCursorEditor P((Editor));
static status		selectedFragmentEditor P((Editor, Fragment));
static status		showMatchingBracketEditor P((Editor, Int));
static status		insertSelfFillEditor P((Editor, Int, Int));
static status		scrollDownEditor P((Editor, Int));
static status		selectionOriginEditor P((Editor, Int));
static status		selectionExtendEditor P((Editor, Int));
static status		selection_editor P((Editor, Int, Int));
static status		selectionToCutBufferEditor P((Editor, Int));
static status		insertCutBufferEditor P((Editor, Int));
static status		insertEditor(Editor e, CharArray str);
static status		lineNumberEditor P((Editor, Int));
static status		saveEditor P((Editor, FileObj));
