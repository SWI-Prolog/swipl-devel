...
#ifdef DND_DEBUG
#define dnd_debug1(a)       printf("%s: %d: " a "\n", __FILE__, __LINE__)
#define dnd_debug2(a,b)     printf("%s: %d: " a "\n", __FILE__, __LINE__ , b)
	      ...
	      int want_position, int x, int y, int w, int h, Atom action)
{
	...
	if (will_accept) {
            dnd_debug2("Sent accept %s", XGetAtomName(dnd->display, action));
...
{   Window owner;
    ...
    if ( !(owner = XGetSelectionOwner (dnd->display, dnd->XdndSelection)) ||
	 owner == None ) {
...
#if XDND_VERSION >= 3
    Window last_toplevel_window = 0, toplevel_window = 0;
#endif
    int n;
    DndCursor *cursor;
	...
	case MotionNotify:
		...
		while (XQueryPointer (dnd->display, xevent.xmotion.subwindow, &root_return, &child_return,
			...
			if ((dnd_aware = xdnd_is_dnd_aware (dnd, xevent.xmotion.subwindow, &dnd->dragging_version, typelist))) {
			    toplevel_window = xevent.xmotion.subwindow;
...
#else
	    if (toplevel_window != last_toplevel_window &&
		last_window != xevent.xmotion.subwindow &&
		(
		    (dnd_aware)
		    ||
...
#else
			    xdnd_send_leave (dnd, last_toplevel_window, from);
#endif
			}
			dnd->internal_drag = 0;
			dnd->dropper_window = 0;
...
#if XDND_VERSION >= 3
		last_toplevel_window = toplevel_window;
#endif
		if (dnd_aware) {
...
#else
			xdnd_send_enter (dnd, toplevel_window, from, typelist);
#endif
		    }
		    dnd->want_position = 1;
		    dnd->ready_to_drop = 0;
...
/* got here, so we are just moving `inside' the same window */
		if (dnd->stage == XDND_DRAG_STAGE_ENTERED) {
			...
			if (dnd->internal_drag) {
			    ...
			    dnd_debug2 ("    not our own widget - sending XdndPosition to %d", (int) over_window);
#if XDND_VERSION < 3
			    xdnd_send_position (dnd, over_window, from, action, dnd->x, dnd->y, xevent.xmotion.time);
#else
			    xdnd_send_position (dnd, toplevel_window, from, action, dnd->x, dnd->y, xevent.xmotion.time);
#endif
			}
		    } else {
			dnd_debug1 ("  doesn't want position or inside rectangle");
		    }
		}
	    }
	    last_window = xevent.xmotion.subwindow;
	...
	case ClientMessage:
		...
		if (dnd->stage == XDND_DRAG_STAGE_ENTERED && XDND_STATUS_TARGET_WIN (&xevent) == dnd->dropper_window) {
		    ...
		    dnd_debug1 ("    XdndStatus stage incorrect or dropper window incorrect");
		}
	    }
	    break;
...
/* error is not actually used, i think future versions of the protocol maybe should return 
   an error status to the calling window with the XdndFinished client message */
	...
	dnd_debug1 ("got ClientMessage");
	if (xevent->xclient.message_type == dnd->XdndEnter) {
	    dnd_debug1 ("  message_type is XdndEnter");
#if XDND_VERSION >= 3
	    ...
	    dnd_debug1 ("  message_type is XdndPosition");
		...
		if (dnd->will_accept) {
		    dnd_debug1 ("    will_accept is true - converting selection");
...
DndClass *
xdnd_default_dnd(Display * display)
...
{
    Atom action = 0;
    DndClass *dnd = xdnd_default_dnd(display);
