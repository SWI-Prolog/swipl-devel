...
COMMON(BoolObj)	getOpenDisplay(Any d);
COMMON(status)	drawInDisplay(DisplayObj d, Graphical gr, Point pos, BoolObj invert, BoolObj subtoo);
COMMON(status)	grabServerDisplay(DisplayObj d, BoolObj val);
...
COMMON(FrameObj) getFrameWindow(PceWindow sw, BoolObj create);
