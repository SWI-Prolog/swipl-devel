...
do_expand((:- pce_begin_class(Spec, Super, Doc)), Decls) :-
	break_class_specification(Spec, ClassName, MetaClass, TermArgs),
	feedback(open_class(ClassName)),
	...
	pce_declarations(Decls).
do_expand((:- pce_extend_class(ClassName)), Decls) :-
	push_class(ClassName),
...
/* using fcompile/1, the class is already there, and this should be
   handled by pce_extended_class or pce_register_class anyway
...
%	pce_declarations/1
%
%	provide declarations for the given file.  TBD: deal with reload!

:- dynamic
	file_declarations_done/1.

pce_declarations([]) :-
	prolog_load_context(file, File),
	file_declarations_done(File), !.
pce_declarations((:- multifile((pce_principal:send_implementation/3,
				pce_principal:get_implementation/4,
				pce_principal:pce_class/6,
				pce_principal:pce_lazy_send_method/3,
				pce_principal:pce_lazy_get_method/3,
				pce_principal:pce_uses_template/2)))) :-
	prolog_load_context(file, File),
	asserta(file_declarations_done(File)).

:- multifile
	user:portray_message/2.

user:portray_message(informational, loading(_How, File)) :-
	retract(file_declarations_done(File)),
%	format('** Cleaned declarations of ~w~n', [File]),
	fail.
user:portray_message(informational, loaded(_How, File, _Time, _Bytes)) :-
	retractall(file_declarations_done(File)),
	fail.
...
template_body(G, T, C, send_class(R, C, Msg)) :-
	prolog_load_context(module, M),
	expand_goal(G, M, send_class(R, T, Msg)), !.
template_body(G, T, C, get_class(R, C, Msg, V)) :-
	prolog_load_context(module, M),
	expand_goal(G, M, get_class(R, T, Msg, V)), !.
template_body(G, _, _, G).

expand_goal(In, M, Out) :-
	user:goal_expansion(In, M, Out0), !, 	% TBD In \=@= Out0!
	expand_goal(Out0, M, Out).
expand_goal(In, _, In).
...
set_attribute(Class, Name, Value) :-
	...
	feedback(set_attribute(Class, Name, Value)).

add_attribute(Class, Name, Value) :-
	assert(attribute(Class, Name, Value)),
	feedback(add_attribute(Class, Name, Value)).
