...
public class Query implements Enumeration {
	 ...
	 * This mode is redundant, deprecated, and retained only for backward compatibility.
	 * 
	 * @return an array of zero or more Hashtables of zero or more variablename-to-term bindings (each Hashtable represents a solution, in the order in which they were found)
	 * 
	...
	/**
	 * Returns the first solution, if any, as a (possibly empty) Hashtable of variablename-to-term bindings.
	 * 
	 ...
	 * and calls it at most once, returning the first solution, if there is one, as a (possibly empty) Hashtable.
	 ...
	 * the resulting goal is then called (at most once) and the first solution, if there is one, is returned as a (possibly empty) Hashtable.
	 * 
	 * Otherwise, if text denotes an atom, this static method creates a Query
	 * where text is the name of the goal and params are the args;
	 * the resulting goal is then called as above.
	...
	/**
	 * JPL will attempt to call this Query's goal within an available Prolog engine.
	 ...
	 * 
	 * @param   text  the name of the principal functor of the Query which is to be called
	 * @param   args  the arguments of the Query which is to be called
	 */
	public static final boolean hasSolution( String text, Term[] args){
		return (new Query(text,args)).hasSolution();
	}
