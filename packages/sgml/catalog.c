...
static void scan_overflow(size_t buflen) {
    fprintf(stderr,
...
static int scan(FILE *src, char *buffer, size_t buflen, int kw_expected) {
    ...
    *p = '\0';
    if (kw_expected) {
        if (ci_streql(buffer, "public"))   return CAT_PUBLIC;
        if (ci_streql(buffer, "system"))   return CAT_SYSTEM;
        if (ci_streql(buffer, "entity"))   return CAT_ENTITY;
        if (ci_streql(buffer, "doctype"))  return CAT_DOCTYPE;
	if (ci_streql(buffer, "override")) return CAT_OVERRIDE;
	if (ci_streql(buffer, "base"))     return CAT_BASE;
    }
    return CAT_OTHER;
}
...
static char *dupstr(char const *s) {
    ...
    if (r == 0) {
        fprintf(stderr, "Memory ran out while reading a catalogue.\n");
        exit(EXIT_FAILURE);
    }
    return strcpy(r, s);
}
...
static void load_one_catalogue(catalog_file *file) {
    ...
    p = base + strlen(base);
    while (p != base && !isDirSep(p[-1])) p--;

    for (;;) {
	t = scan(src, buffer, sizeof buffer, 1);
	switch (t) {
	    case CAT_BASE:
	        if (scan(src, buffer, sizeof buffer, 0) == EOF) break;
	        (void)strcpy(base, buffer);
	        p = base + strlen(base);
	        if (p != base && !isDirSep(p[-1])) *p++ = '/';
	        continue;
	    case CAT_OVERRIDE:
	        if (scan(src, buffer, sizeof buffer, 0) == EOF) break;
	    ...
	    case CAT_ENTITY:
	    case CAT_DOCTYPE:
		this_item = malloc(sizeof *this_item);
		if (this_item == 0) {
		    fprintf(stderr,
			 "Memory ran out while reading a catalogue.\n");
		    exit(EXIT_FAILURE);
		}
	        if (scan(src, buffer, sizeof buffer, 0) == EOF) break;
		if (t == CAT_PUBLIC) squish(buffer);
	        this_item->next = 0;
		    ...
		    this_item->replacement = dupstr(base);
		} else {
		    this_item->replacement = dupstr(buffer);
		}
...
char const *find_in_catalogue(
    ...
    if (kind == CAT_OTHER) return 0;

    item = malloc(sizeof *item);
    if (item == 0) {
	fprintf(stderr, "Memory exhausted while growing catalogue.\n");
	exit(EXIT_FAILURE);
    }
    item->next = 0;
    item->kind = kind;
    item->target = dupstr(name);
    switch (kind) {
	case CAT_DOCTYPE:
	...
	case CAT_PENTITY:
	    ...
	    (void)sprintf(penname, "%s.pen", item->target+1);
	    break;
	case CAT_ENTITY:
    ...
    item->replacement = dupstr(penname);
    if (first_item == 0) {
	first_item = item;
    } else {
	last_item->next = item;
