...
\begin{enumerate}
    \item Use I/O multiplexing based on \index{wait_for_input/3}\predref{wait_for_input}{3}.  On Windows
...
IP-address of the client.
    ...
    \predicate{tcp_connect}{2}{+Socket, +Host:+Port}
...
(i.e. 192.168.1.255).  See \index{udp_send/4}\predref{udp_send}{4}.
...
Similar to \index{tcp_socket/1}\predref{tcp_socket}{1}, but create a socket using the \const{SOCK_DGRAM}
...
Prolog string object (see \index{string_to_list/2}\predref{string_to_list}{2}). \arg{From} is a term of the
format \mbox{ip(\arg{A},\arg{B},\arg{C},\arg{D}):\arg{Port}} indicating
the sender of the message. \arg{Options} is currently unused.
\arg{Socket} can be waited for using \index{wait_for_input/3}\predref{wait_for_input}{3}.
...
Host is either the hostname or a term \index{ip/4}\predref{ip}{4}.  \arg{Options} is currently unused.
...
address as a \index{ip/4}\predref{ip}{4} term.
\end{description}
...
using \index{remove_alarm/1}\predref{remove_alarm}{1}.
\end{description}
...
True if Goal completes within Time. \arg{Goal} is executed as in \index{once/1}\predref{once}{1}.
If \arg{Goal} doesn't complete within \arg{Time} seconds, exit using the
exception \const{time_limit_exceeded}. See \index{catch/3}\predref{catch}{3}.
