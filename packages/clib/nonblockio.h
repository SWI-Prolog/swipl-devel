...
NBIO_EXPORT(nbio_sock_t) nbio_socket(int domain, int type, int protocol);
...
NBIO_EXPORT(nbio_sock_t) nbio_accept(nbio_sock_t master,
...
extern const char*	nbio_last_error(nbio_sock_t socket);
NBIO_EXPORT(int)	nbio_setopt(int socket, nbio_option opt, ...);
extern int		nbio_get_flags(int socket);

					/* support tipc_setopt() */
extern plsocket_ptr	nbio_to_plsocket(nbio_sock_t socket);
...
NBIO_EXPORT(int)	tcp_get_socket(term_t Socket, int *id);
NBIO_EXPORT(int)	tcp_unify_socket(term_t Socket, int id);
