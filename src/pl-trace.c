...
{ GET_LD
  ...
  *nextref = parentFrame(fr);
  if (*nextref == NULL) /* Finished */
  {
     return 0;
  }
  ...
  { pc = fr->programPointer - fr->parent->clause->clause->codes;
     return snprintf(buf, len, "%s [PC=%p] [Clause %d]", predicateName(fr->predicate), pc, clauseNo(fr->predicate, fr->clause->clause));
  } else 
