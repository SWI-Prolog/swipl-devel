...
Each goal is printed using the Prolog predicate write_term/2. The style is defined by the Prolog flag [`debugger_write_options`](#flags) and can be modified using this flag or using the [`w`, `p` and `d` commands of the tracer](#trace-formatting-commands).
