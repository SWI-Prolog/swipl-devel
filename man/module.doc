...
the module \pllib{backcomp} without affecting existing code. This
...
and autoload/2 that behave similar to use_module/[1,2], but do not
perform the actual loading.  The generic autoloader now proceeds as
follows if a missing predicate is encountered:
...
Only 0..9, \chr{:}, \chr{^} and \const{//} are interpreted; the mode
declarations \chr{+}, \chr{-}, \chr{*} and \chr{?} are ignored.
...
module sensitive.  The \chr{*} notation is an alias for \chr{?} for
compatibility with e.g., Logtalk.  The specific mode has merely
documentation value.  See \secref{argmode} for details.
    \end{description}
...
predicates.  All formats accepted by use_module/2 for \arg{Import}
are accepted.  The reexport declarations must immediately follow the
module declaration.
\end{description}
...
In general, the import relations between modules form an acyclic
directed graph. The import relation affects the following mechanisms:

\begin{description}
    \item [Predicate visibility]
...
\const{user} and \const{system} modules (in that order).

    \item [Operators]
Operators are also searched through the import relations.  System
operators are defined in the module \const{system}.  The user may
define operators in \const{user} to make them globally visible for
...
exported using the module/2 module header.

    \item [The \prologflag{unknown} flag]
This flag controls the response to encountering an undefined predicate
in the target module.

    \item [Term and goal expansion]
The hooks term_expansion/2 and goal_expansion/2 (see
\secref{progtransform}) are \jargon{chained} over the import modules
that define these hooks.  This implies we collect all modules that
provide definitions for these hook predicates by traversing the
import module relation depth-first and left-to-right.  Next, we
perform the transformations in a \jargon{pipeline}, starting at the
target module.
\end{description}

The list of import modules for a specific module can be manipulated and
queried using the following predicates, as well as using set_module/1.
...
predicates. Module \const{system} has no import module, i.e., is a
\jargon{root} of the module graph. The second special module is the
module \const{user}. This module forms the initial working space of the
user. Initially it is empty.\footnote{Unfortunately some \jargon{hooks}
are traditionally defined in the user module}. The import module of
module \const{user} is \const{system}, making all built-in predicates
available.

All normal application modules import from the module \const{user}. This
implies they can use all predicates imported into \const{user} without
explicitly importing them. If an application loads all modules from the
...
All \jargon{library} modules (see module_property/2) import directly
from \const{system}. Library modules are modules loaded from the
SWI-Prolog installation. As they import from \const{system}, the
functionality of a library is not affected by operator or predicate
definitions in the \const{user} module.
