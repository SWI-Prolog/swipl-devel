...
distinction between atoms and the empty list. As a result, we cannot
use type polymorphism that involve both atoms and lists. For example, we
cannot use \jargon{multi lists} (arbitrary deeply nested lists) of
...
requires `opening' proper lists (i.e., copying the list while replacing
the terminating atom \verb$'[]'$ with a variable) that have to be added
to the sequence. The ambiguity of atom and list is particularly painful
when mapping external data representations that do not suffer from this
ambiguity.
...
and the result is converted to \arg{String}.  If \arg{String} only
contains white space and/or comments, an \term{syntax_error}{end_of_string}
exception is raised.
...
codes. At least one of the two arguments must be instantiated.
    ...
    \predicate{string_bytes}{3}{?String, ?Bytes, +Encoding}
True when the (Unicode) \arg{String} is represented by \arg{Bytes} in
\arg{Encoding}. If \arg{String} is instantiated it may represent text as
an atom, string, list of character codes or list or characters.
\arg{Bytes} is always a list of integers in the range $0\ldots{}255$. At
least one of \arg{String} or \arg{Bytes} must be instantiated. This
predicate is notably intended as an intermediate step to perform byte
oriented operations on text. Examples are (base64) encoding, encryption,
computing a (secure) hash, etc. \arg{Encoding} is typically
\const{utf8}. All valid stream encodings except for \const{wchar_t}
are supported.  See \secref{encoding}.  Note that this translation
is only provided for strings.  Creating an atom from bytes requires
atom_string/2.\footnote{Strings are an efficient intermediate and this
conversion is needed only in some uncommon scenarios.}
...
\jargon{anytext} as its first argument. Numeric types are formatted
into strings before the length of their string representation is
determined.\footnote{This behavior should be considered deprecated}
See also write_size/4.
...
The next example defines a predicate that inserts a value at a position.
See sub_atom/5 for more examples.
...
Read at most \arg{Length} characters from \arg{Stream} and return them
in the string \arg{String}. If \arg{Length} is unbound, \arg{Stream} is
read to the end and \arg{Length} is unified with the number of
characters read. The number of \jargon{bytes} read depends on the
encoding of \arg{Stream} (see \secref{encoding}). This predicate may be
used to read a sequence of bytes when the stream is in \const{octet}
encoding.   See open/4 and set_stream/2 for controlling the encoding.
...
\arg{String}. \arg{String} can be any text representation, i.e., string,
atom, list of codes or list of characters. The created \arg{Stream} has
the \const{reposition} property (see stream_property/2). Note that the
internal encoding of the data is either ISO Latin 1 or UTF-8.
\end{description}
...
\secref{string} and \secref{ext-dquotes-motivation}.  To
...
illustrated below. \arg{Tag} is an atom.\footnote{That \arg{Tag} can
be a variable. This used to be common for jargon{anonymous} dicts, but
is now deprecated in favour of the reserved tag \chr{#}.} As with
...
Two dicts unify only if they have the same tag and the same set of keys
and values associated with the keys unify.  As normal in Prolog, after
successful unification the two dicts are indistingsable according to
\predref{==}{2} and compare/3.

\subsection{Dict types and tags}
\label{sec:ext-dict-compat}

The \href{https://prolog-lang.org/ImplementersForum/PIPs.html}{PIP}
...
consensus regarding the \verb$Tag{...}$ syntax:

\begin{itemize}
    \item The \ldots in \verb$Tag{...}$ is a set of \arg{Key}:\arg{Value},
where \arg{Key} is an atom (and possibly (small) integer) and
\arg{Value} is any term.   This is compatible to SWI-Prolog.
    \item The Tag is either
    \begin{itemize}
        \item The atom \chr{#}.  This denotes a \jargon{dynamic dict}, which
	is what our dicts are.
	\item Any other atom.  This denotes a \jargon{named arguments} or
	\jargon{struct}.  This syntax is mapped to a compound term whose
	arity and allowed keys is defined by a declaration.
	\item A variable.  This is parsed into an \jargon{attributed variable}
	(see \secref{attvar}).
    \end{itemize}
\end{itemize}

Our aim is to support these PIPs. Next to mapping ``any other atom'' to
a named argument structure, we will support mapping these to a dynamic
dict, optionally with user-defined functions (see
\secref{ext-dict-user-functions}).  The transition is guided by the
Prolog flag \prologflag{var_tag}. Its current default (\const{dict}) is
compatible with the original dict implementation of SWI-Prolog.

The use of unbound tags is deprecated.  It is recommended to use \chr{#}
for anonymous dynamic dicts.  The main reason for using unbound tags is
in using them with the predicates \predref{:<}{2} and \predref{>:<}{2}
to access a set of keys in a dict with unknown tag.   In the current
version the \chr{#} tag matches any tag in these predicates. For
example, to extract the \const{x} and \const{y} from a dict, we can
now use
...
\begin{description}
    \dictfunction{get}{1}{?KeyPath}
Return the value associates with \arg{KeyPath}. \arg{KeyPath} is either
a single key or a term \exam{Key1/Key2/...}. Each key is either an atom,
small integer or a variable. While \exam{Dict.Key} throws an existence
error, this function \emph{fails} silently if a key does not exist in
the target dict. See also \predref{:<}{2}, which can be used to test for
existence and unify multiple key values from a dict. For example:
...
or extend the key-values in the original dict.  See put_dict/3.
    ...
    \dictfunction{get}{2}{?KeyPath, +Default}
Same as \nopredref{get}{1}, but if no match is found the function
evaluates to \arg{Default}.  If \arg{KeyPath} contains variables
possible choice points are respected and the function only evaluates
to \arg{Default} if the pattern has no matches.
...
True when \arg{Select} is a `sub dict' of \arg{From}: the tags must
match and all keys in \arg{Select} must appear with unifying values in
\arg{From}. Two tags match if the unify or one of the tags is the
reserved \chr{#} tag. \arg{From} may contain keys that are not in
\arg{Select}. This operation is frequently used to \emph{match} a dict
and at the same time extract relevant values from it. For example:
...
True when the tags of \arg{Select} and \arg{From} match,
...
appear in \arg{Select} are used to form an anonymous dict, which is
...
\arg{Dict1} and \arg{Dict2}. It is true when the tags match and the
values associated with all \emph{common} keys have been unified. The
...
comprehension} \cite{declarativeloops:2010}.
